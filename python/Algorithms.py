class aes_encrypt:
    ...
    def multi(self, n1, n2):
        ...
        if n1 or n2 == "0x00":
            ...
            out1 = int("0x" + (L_box[n1[-2]])[n1[-1]], 16) + int(
                "0x" + (L_box[n2[-2]])[n2[-1]], 16
            )
            if out1 > 255:
                out1 -= 255
            out1 = hex(out1)
    ...
    def MixColumns(self, statearray, I=False):
        ...
        for i in range(0, len(statearray)):
            statearray[i][0] = hex(
                self.multi(statearray[i][0], Multi_Matrix[i][0])
                ^ self.multi(statearray[i][1], Multi_Matrix[i][1])
                ^ self.multi(statearray[i][2], Multi_Matrix[i][2])
                ^ self.multi(statearray[i][3], Multi_Matrix[i][3])
            )
